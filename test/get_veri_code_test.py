...
def get_extension_path():
    ...
    print(f"Trying to load extension path: {extension_path}")

    if not os.path.exists(extension_path):
        raise FileNotFoundError(
            f"Extension does not exist: {extension_path}\nPlease make sure the turnstilePatch folder is in the correct location"
        )
...
def get_veri_code(username):
    ...
    try:
        ...
        # Wait and get new mail
        while True:
            new_mail = tab.ele("@class=mail")
            if new_mail:
                if new_mail.text:
                    print("Latest mail:", new_mail.text)
