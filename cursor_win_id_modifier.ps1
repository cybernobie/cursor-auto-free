...
function Modify-CursorJSFiles {
    ...
    Write-Host "$GREEN🔑 [Generated]$NC New device identifier generated"
    ...
    if (-not $needModification) {
        Write-Host "$GREEN✅ [Skip]$NC All JS files have already been modified, no need to repeat"
        return $true
    }

    # 关闭Cursor进程
    Write-Host "$BLUE🔄 [Close]$NC Closing Cursor process to modify files..."
    ...
    foreach ($file in $jsFiles) {
        ...
        try {
            ...
            if ($content -match "return crypto\.randomUUID\(\)" -or $content -match "// Cursor ID 修改工具注入") {
                Write-Host "$GREEN✅ [Skip]$NC File has already been modified"
                $modifiedCount++
                continue
            }
...
"@
            ...
            Write-Host "$BLUE🔍 [Found]$NC Found device ID related function"
                ...
                Write-Host "$YELLOW⚠️  [Warning]$NC Known device ID function pattern not found, using generic injection"
                $content = $injectCode + $content
                $modifiedCount++
            }
